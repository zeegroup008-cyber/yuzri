...
const PPOB_COLORS = {
  "telkomsel-pulsa":   "#e02020",
  "xl-pulsa":          "#0073e6",
  "indosat-pulsa":     "#f5a623",
  "token-listrik-pln": "#1e88e5",
  "tri-pulsa":         "#6d3fa3",
  "smartfren-pulsa":   "#e53935",
}

function fmt(n) { return "Rp " + Number(n).toLocaleString("id-ID") }

function PPOBCard({ product: p, onClick, index }) {
  const accent = PPOB_COLORS[p.category] || "#7c5cfc"
  ...
  const load = async () => {
    ...
    try {
      const prods = await pb.collection("products").getFullList({
        filter: "type = 'ppob' && is_active = true",
    ...
    <section className="px-4 md:px-8 lg:px-12 py-4 pb-16">
      <div className="flex items-center gap-4 mb-10">
        ...
        <div className="flex items-center gap-2 px-5 py-2 rounded-full" style={{ background: "rgba(124,92,252,0.1)", border: "1px solid rgba(124,92,252,0.25)" }}>
          ...
          <span className="text-xs font-extrabold tracking-widest" style={{ color: "#c4b5fd" }}>PULSA & PPOB</span>
        </div>
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <h2 className="font-syne font-extrabold text-2xl text-white tracking-tight">Top Up Pulsa & Utilitas</h2>
            <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full"
              style={{ background: "rgba(124,92,252,0.2)", color: "#a78bfa", border: "1px solid rgba(124,92,252,0.3)" }}>
              {products.length} PRODUK
            </span>
          </div>
          <p className="text-white/35 text-sm ml-4">Pulsa, paket data & token listrik · Proses instan</p>
        </div>
      ...
      {loading ? (
        ...
        <ErrorState message="Gagal memuat produk PPOB." onRetry={load} />
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {(showAll ? products : products.slice(0, 12)).map((p, i) => (
            <PPOBCard key={p.id} product={p} index={i} onClick={() => navigate(`/ppob/${p.category}`)} />
          ))}
        </div>
      )}
