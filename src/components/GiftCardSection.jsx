...
const HOT_GIFTCARD = ["steam-wallet", "google-play", "app-store"]

function fmt(n) { return "Rp " + Number(n).toLocaleString("id-ID") }

function GiftCardCard({ product: p, onClick, index }) {
  const accent = GIFTCARD_COLORS[p.category] || "#7c5cfc"
  const isHot  = HOT_GIFTCARD.includes(p.category)
    ...
    <section className="px-4 md:px-8 lg:px-12 py-4 pb-16">
      <div className="flex items-center gap-4 mb-10">
        ...
        <div className="flex items-center gap-2 px-5 py-2 rounded-full" style={{ background: "rgba(124,92,252,0.1)", border: "1px solid rgba(124,92,252,0.25)" }}>
          ...
          <span className="text-xs font-extrabold tracking-widest" style={{ color: "#c4b5fd" }}>GIFT CARD</span>
        </div>
      ...
      <div className="flex items-end justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <h2 className="font-syne font-extrabold text-2xl text-white tracking-tight">Gift Card Digital</h2>
            <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full"
              style={{ background: "rgba(124,92,252,0.2)", color: "#a78bfa", border: "1px solid rgba(124,92,252,0.3)" }}>
              {products.length} PRODUK
            </span>
          </div>
          <p className="text-white/35 text-sm ml-4">Kode dikirim ke email · Proses cepat</p>
        </div>
      ...
      {loading ? (
        ...
        <ErrorState message="Gagal memuat gift card." onRetry={load} />
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {(showAll ? products : products.slice(0, 12)).map((p, i) => (
            <GiftCardCard key={p.id} product={p} index={i} onClick={() => navigate(`/giftcard/${p.category}`)} />
          ))}
        </div>
      )}
      ...
      {!loading && !error && !showAll && products.length > 12 && (
        <div className="text-center mt-8">
          <button onClick={() => setShowAll(true)}
            ...
            Tampilkan Semua {products.length} Gift Card
          </button>
        </div>
      )}
    </section>
