...
export default function PremiumDetailPage() {
  ...
  if (notFound || !produk) return (
    <div className="min-h-[60vh] flex items-center justify-center text-muted">
      ...
      <button onClick={() => navigate("/premium")} className="ml-2 text-purple-2 underline">Kembali</button>
    </div>
  )
  ...
  const validate = () => {
    const e = {}
    if (!email.trim()) e.email = "Alamat email wajib diisi."
    else if (!/\S+@\S+\.\S+/.test(email)) e.email = "Format email tidak valid."
    if (!paket) e.paket = "Pilih paket langganan terlebih dahulu."
  ...
  const handleBayar = async () => {
    ...
    try {
      ...
      formData.append("target_id",      email)
  ...
  return (
    <div className="px-4 md:px-8 lg:px-12 py-10">
      ...
      <div className="flex items-center gap-2 mb-8">
        {["Pilih Paket", "Konfirmasi", "Selesai"].map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step > i + 1 ? "bg-green-500 text-white" : step === i + 1 ? "bg-purple text-white" : "bg-white/10 text-muted"}`}>
              {step > i + 1 ? "?" : i + 1}
            </div>
      ...
      {step === 1 && (
        <div className="space-y-5">
          <div className="bg-card border border-border rounded-2xl p-5">
            <h2 className="font-semibold text-white mb-3">Alamat Email Akun <span className="text-red-400">*</span></h2>
            <input type="email" value={email}
              onChange={(e) => { setEmail(e.target.value); setErrors((p) => ({ ...p, email: "" })) }}
              placeholder="email@contoh.com"
              className={`w-full bg-bg-2 border rounded-xl px-4 py-3 text-white text-sm outline-none placeholder:text-muted transition-colors ${errors.email ? "border-red-500" : "border-border focus:border-purple"}`} />
            {errors.email && <p className="text-red-400 text-xs mt-1">{errors.email}</p>}
            <p className="text-muted text-xs mt-1.5">Akun akan diaktifkan pada email ini. Pastikan emailnya benar.</p>
          </div>

          <div className="bg-card border border-border rounded-2xl p-5">
            <h2 className="font-semibold text-white mb-3">Pilih Paket <span className="text-red-400">*</span></h2>
            <div className="space-y-2">
              {paketList.map((p) => (
                <button key={p.id} onClick={() => { setPaket(p); setErrors((prev) => ({ ...prev, paket: "" })) }}
                  className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${paket?.id === p.id ? "border-purple bg-purple/10" : "border-border bg-bg-2 hover:border-purple/50"}`}>
                  <span className="text-white text-sm font-medium">{p.label}</span>
                  <span className="text-purple-2 font-bold text-sm">{fmt(p.price)}</span>
                </button>
              ))}
            </div>
      ...
      {step === 3 && (
        <div className={`border rounded-2xl p-8 text-center bg-card ${trxStatus === "Approved" ? "border-green-500/30" :
          ...
          {trxStatus === "Approved" && (
            <>
              ...
              <p className="text-muted text-sm mb-1">Paket <span className="text-white font-medium">{paket?.label}</span> telah aktif.</p>
              <p className="text-muted text-sm mb-6">Detail dikirimkan ke <span className="text-white font-medium">{email}</span></p>
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={() => navigate("/premium")} className="bg-purple hover:bg-purple-3 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors">Beli Lagi</button>
              </div>
            </>
          )}
          {trxStatus === "Rejected" && (
            <>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={() => navigate("/premium")} className="bg-purple hover:bg-purple-3 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors">Kembali</button>
              </div>
            </>
          )}
        </div>
