...
const VOUCHER_DATA = {
  "roblox": {
    name: "Roblox", type: "Robux", color: "from-red-600 to-red-900", icon: "🎲",
    pakets: [
      { id: 1, label: "400 Robux",  harga: 55000  },
      { id: 2, label: "800 Robux",  harga: 108000 },
      { id: 3, label: "1700 Robux", harga: 215000 },
      { id: 4, label: "4500 Robux", harga: 560000 },
    ],
  },
  "valorant": {
    name: "Valorant", type: "VP (Valorant Points)", color: "from-pink-600 to-red-900", icon: "🔫",
    pakets: [
      { id: 1, label: "420 VP",   harga: 50000  },
      { id: 2, label: "1000 VP",  harga: 115000 },
      { id: 3, label: "2050 VP",  harga: 225000 },
      { id: 4, label: "3650 VP",  harga: 395000 },
      { id: 5, label: "5350 VP",  harga: 565000 },
      { id: 6, label: "11000 VP", harga: 1120000 },
    ],
  },
  "minecraft": {
    name: "Minecraft", type: "Minecoins", color: "from-green-700 to-green-900", icon: "⛏️",
    pakets: [
      { id: 1, label: "320 Minecoins",  harga: 38000  },
      { id: 2, label: "840 Minecoins",  harga: 96000  },
      { id: 3, label: "1720 Minecoins", harga: 190000 },
      { id: 4, label: "3500 Minecoins", harga: 380000 },
    ],
  },
  "fortnite": {
    name: "Fortnite", type: "V-Bucks", color: "from-cyan-500 to-blue-900", icon: "🏆",
    pakets: [
      { id: 1, label: "1000 V-Bucks",  harga: 128000 },
      { id: 2, label: "2800 V-Bucks",  harga: 320000 },
      { id: 3, label: "5000 V-Bucks",  harga: 560000 },
      { id: 4, label: "13500 V-Bucks", harga: 1400000 },
    ],
  },
}
...
export default function VoucherDetailPage() {
  ...
  const voucher = VOUCHER_DATA[produkId]
  ...
  const paketList = paketsDB.length > 0 ? paketsDB : (voucher?.pakets || [])
  ...
  if (!voucher) return (
    <div className="min-h-[60vh] flex items-center justify-center text-muted">
      ...
      <button onClick={() => navigate("/voucher")} className="ml-2 text-purple-2 underline">Kembali</button>
    </div>
  )

  const validate = () => {
    const e = {}
    if (!userId.trim()) e.userId = "ID Akun wajib diisi."
    if (!paket)         e.paket  = "Pilih nominal voucher terlebih dahulu."
  ...
  const handleBayar = async () => {
    ...
    try {
      ...
      formData.append("product_name",   `${voucher.name} — ${paket.label}`)
  ...
  return (
    <div className="px-4 md:px-8 lg:px-12 py-10">
      ...
      <div className={`bg-gradient-to-r ${voucher.color} rounded-2xl p-6 mb-6 flex items-center gap-4`}>
        <span className="text-4xl">{voucher.icon}</span>
        <div>
          <h1 className="font-syne font-bold text-xl text-white">{voucher.name}</h1>
          <p className="text-white/70 text-sm">Voucher {voucher.type}</p>
        </div>
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div className="bg-card border border-border rounded-2xl p-5">
            <h2 className="font-semibold text-white mb-3">Data Akun</h2>
            <div>
              <label className="block text-sm text-white/70 mb-1.5 font-medium">ID Akun <span className="text-red-400">*</span></label>
              <input value={userId} onChange={(e) => { setUserId(e.target.value); setErrors((p) => ({ ...p, userId: "" })) }}
                placeholder="Masukkan ID akun kamu"
      ...
      {step === 2 && (
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          <div className="divide-y divide-border">
            {[
              { label: "Produk",     value: voucher.name },
              { label: "Nominal",    value: paket.label },
              { label: "ID Akun",    value: userId },
        ...
        <div className={`border rounded-2xl p-8 text-center bg-card ${
          ...
          {trxStatus === "Approved" && (
            <>
              ...
              <p className="text-muted text-sm mb-1">{paket?.label} dikirim ke akun <span className="text-white font-medium">{userId}</span></p>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={() => navigate("/voucher")} className="bg-purple hover:bg-purple-3 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors">Beli Lagi</button>
              </div>
            </>
          )}
          {trxStatus === "Rejected" && (
            <>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={() => navigate("/voucher")} className="bg-purple hover:bg-purple-3 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-colors">Kembali</button>
              </div>
            </>
          )}
        </div>
