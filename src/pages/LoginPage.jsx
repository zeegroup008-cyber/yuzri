...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setError("Login dengan Google gagal. Pastikan Google OAuth sudah dikonfigurasi di PocketBase.")
    } finally {
      setGLoading(false)
    }
  }
      ...
      <div className="w-full max-w-md">
        <div className="bg-card border border-border rounded-2xl p-8">
          <div className="text-center mb-8">
            <h1 className="font-syne font-bold text-2xl text-white mb-2">Selamat Datang Kembali</h1>
            <p className="text-muted text-sm">Masuk untuk melanjutkan ke akun Anda</p>
          </div>
          ...
          >
            ...
            {gLoading ? "Menghubungkan..." : "Masuk dengan Google"}
          </button>

          {/* Divider */}
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.08)" }} />
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>atau masuk dengan email</span>
            <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.08)" }} />
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm text-white/70 mb-1.5 font-medium">Alamat Email</label>
              <input
                type="email"
                required
                value={form.email}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-sm text-white/70 font-medium">Kata Sandi</label>
                <a href="#" className="text-purple-2 text-xs hover:text-purple transition-colors">Lupa kata sandi?</a>
              </div>
              <input
                type="password"
                required
          ...
          <p className="text-center text-sm text-muted mt-6">
            Belum punya akun?{" "}
            <Link to="/daftar" className="text-purple-2 hover:text-purple font-medium no-underline transition-colors">
              Daftar sekarang
            </Link>
          </p>
        </div>
