...
const PPOB_COLORS = {
  "telkomsel-pulsa":  { accent: "#e02020" },
  "xl-pulsa":         { accent: "#0073e6" },
  "indosat-pulsa":    { accent: "#f5a623" },
  "token-listrik-pln":{ accent: "#1e88e5" },
  "tri-pulsa":        { accent: "#6d3fa3" },
  "smartfren-pulsa":  { accent: "#e53935" },
}
...
export default function PPOBPage() {
  ...
  return (
    <div className="px-4 md:px-8 lg:px-12 py-12">
      <h1 className="font-syne font-bold text-3xl text-[var(--color-text)] mb-2">PPOB</h1>
      <p className="text-[#8888aa] mb-8">Pulsa, paket data, dan token listrik dengan harga terjangkau.</p>
      {loading ? (
        <div className="text-[#8888aa] text-sm">Memuat produk...</div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {products.map((p) => {
            const colors = PPOB_COLORS[p.category] || { accent: "#7c5cfc" }
            return (
              <div key={p.id} onClick={() => navigate(`/ppob/${p.category}`)}
                ...
                <div className="p-3" style={{ background: "#181929", borderTop: `2px solid ${colors.accent}` }}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/ppob/${p.category}`) }}
